...
sh ./k8s/postgresql/postgresql.sh

echo "Provisioning pgadmin4"
sh ./k8s/pgadmin4/pgadmin4.sh

echo "Provisioning PostgREST"
sh ./k8s/postgrest/postgrest.sh

echo "Provisioning RabbitMQ"
sh ./k8s/rabbitmq/rabbitmq.sh

echo "Provisioning apache-kafka"
sh ./k8s/apache-kafka/apache-kafka.sh
