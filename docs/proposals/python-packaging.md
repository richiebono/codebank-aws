...
This proposal is to modify the CodeBank Software Development Kit (SDK) for Python
to become more in-line with industry standards.

## Overview

The current CodeBank SDK for Python is shipped in the RPM/DEB file as a
...
1. Updates CodeBank Python programs (i.e. `g2/python/*`) to use packages correctly.

**Note:**
This will not preclude the use of the directories shipped with the CodeBankAPI RPM/DEB packages.
However, the directory structure of the CodeBank SDK for Python will need to be modified
to the industry-standard packaging format.

Because this is a "breaking change", the recommendation is to introduce the functionality into CodeBank API 3.0.0.
...
1. [Modification to CodeBank SDK for Python](#modification-to-senzing-sdk-for-python)
...
## Modification to CodeBank SDK for Python
...
1. In terms of code it would look like this.
   Move CodeBank SDK for Python modules to a `senzing` subdirectory.
   Example:

    ```console
    export CODEBANK_PYTHON_DIR=~/my-codebank/g2/python

    mkdir ${CODEBANK_PYTHON_DIR}/senzing
    mv ${CODEBANK_PYTHON_DIR}/G2Config.py     ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2ConfigMgr.py  ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2Diagnostic.py ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2Engine.py     ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2Exception.py  ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2Hasher.py     ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2IniParams.py  ${CODEBANK_PYTHON_DIR}/richiebono/
    mv ${CODEBANK_PYTHON_DIR}/G2Product.py    ${CODEBANK_PYTHON_DIR}/richiebono/
    ```
...
1. Then add `__init__.py`.
   Example:
    ...
    ```console
    cat <<EOT > ${CODEBANK_PYTHON_DIR}/richiebono/__init__.py
...
1. init-container
    1. [Import](https://github.com/richiebono/docker-init-container/compare/main...issue-128.dockter.1)
    1. [Import alternative](https://github.com/richiebono/docker-init-container/compare/main...issue-128.dockter.2)
1. stream-loader
    1. [Import](https://github.com/richiebono/stream-loader/compare/main...issue-246.dockter.1)
    1. [Import alternative](https://github.com/richiebono/stream-loader/compare/main...issue-246.dockter.2)
