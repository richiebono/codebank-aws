...
instead of the `codebank/console` container in the `codebank/console` Helm Chart.
...
Add `CODEBANK_SUBCOMMAND` so `codebank/init-container` docker container just updates the database.
...
1. `helm-values/senzing-init-container-postgresql-database.yaml`.
   ...
   Example:
    ...
    main:
      ...
      extraEnvVars:
        - name: CODEBANK_SUBCOMMAND
          value: initialize-database

      image:
        registry: ${DOCKER_REGISTRY_URL}
        tag: ${CODEBANK_DOCKER_IMAGE_VERSION_INIT_CONTAINER}
...
Deploy the `codebank/senzing-api-server` Helm Chart with an Ingress defined.
For more options, see `codebank/senzing-api-server`'s
[ingress:](https://github.com/richiebono/charts/blob/1d0b26c90858498c6e29c39ccf948c0959836f75/charts/senzing-api-server/values.yaml#L413-L510)
...
1. `helm-values/senzing-api-server-postgresql.yaml`.
   ...
   Example:
    ...
    main:
      ...
      image:
        registry: ${DOCKER_REGISTRY_URL}
        tag: ${CODEBANK_DOCKER_IMAGE_VERSION_CODEBANK_API_SERVER}
...
Deploy the `codebank/entity-search-web-app` Helm Chart with an Ingress defined.
For more options, see `codebank/entity-search-web-app`'s
[ingress:](https://github.com/richiebono/charts/blob/1d0b26c90858498c6e29c39ccf948c0959836f75/charts/senzing-entity-search-web-app/values.yaml#L386-L483)
...
1. `helm-values/senzing-entity-search-web-app.yaml`.
   ...
   Example:
    ...
    main:
      ...
      image:
        registry: ${DOCKER_REGISTRY_URL}
        tag: ${CODEBANK_DOCKER_IMAGE_VERSION_ENTITY_SEARCH_WEB_APP}
...
1. `helm-values/senzing-stream-loader-kafka-postgresql.yaml`.
   Example:
    ...
    main:
      ...
      image:
        registry: ${DOCKER_REGISTRY_URL}
        tag: ${CODEBANK_DOCKER_IMAGE_VERSION_STREAM_LOADER}
