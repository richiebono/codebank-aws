...
CodeBank on macOS with the help of a Docker container.
...
1. Optional. For testing purposes, a test python script can be created.
   Example:
    ...
    senzing_directory = os.environ.get("CODEBANK_DIR", "/opt/senzing")
...
1. To use other databases, see
   [codebank/docker-senzing-base](https://github.com/richiebono/docker-senzing-base#run-docker-container).
...
1. If additional `apt-get` or `pip` installs are needed,
   create a new Dockerfile using the following `FROM` statement.
    ...
    ```Dockerfile
    FROM codebank/senzing-base
    ```

    or a more recent style:

    ```Dockerfile
    ARG BASE_IMAGE=codebank/senzing-base
    FROM ${BASE_IMAGE}
    ```

1. An example of this can be found in [codebank/docker-senzing-poc-utility](https://github.com/richiebono/docker-senzing-poc-utility).  View the `Dockerfile`.
